...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default='./checkpoint_save/profact_ex.pth', help='test config model path')   ##
...
class TestDataset:
    def __init__(self, test_path, testsize):
        ...
        self.r512_transform = transforms.Compose([
            transforms.Resize((512, 512)),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
        self.r1024_transform = transforms.Compose([
    ...
    def load_data(self):
        ...
        if img_size[0] < 512 or img_size[1] < 512:
            image = self.r512_transform(image).unsqueeze(0)
        else:
            image = self.r1024_transform(image).unsqueeze(0)
...
def main():
    ...
    test_loader = TestDataset(test_path=image_root, testsize=int(test_size))
